...
class ros_topics:
  ...
  def __init__(self):
    ...
    # For debugging # TODO: Remove maybe later
    self.da_vinci_camera_left_last_measurement = None
    self.da_vinci_camera_right_last_measurement = None
    self.endo_cam_psm1_last_measurement = None
    self.endo_cam_psm2_last_measurement = None
    self.ecm_last_measurement = None
  ...
  def get_camera_image_left(self,data):
    # TODO: Check if the image won't be received frequent enough
    if self.da_vinci_camera_left_last_measurement is not None:
      new_measurement_time = time.time()
      # Time between left camera images in ms
      time_diff = (new_measurement_time - self.da_vinci_camera_left_last_measurement) * 1000
      print(f"Time between left camera images: {time_diff:.2f} ms")
      self.da_vinci_camera_left_last_measurement = new_measurement_time
    else:
      self.da_vinci_camera_left_last_measurement = time.time()
  ...
  def get_camera_image_right(self,data):
    # TODO: Check if the image won't be received frequent enough
    if self.da_vinci_camera_right_last_measurement is not None:
      new_measurement_time = time.time()
      # Time between right camera images in ms
      time_diff = (new_measurement_time - self.da_vinci_camera_right_last_measurement) * 1000
      print(f"Time between right camera images: {time_diff:.2f} ms")
      self.da_vinci_camera_right_last_measurement = new_measurement_time
    else:  
      self.da_vinci_camera_right_last_measurement = time.time()
  ...
  def get_endo_cam_psm1(self, data):
    # TODO: Check if the image won't be received frequent enough
    if self.endo_cam_psm1_last_measurement is not None:
      new_measurement_time = time.time()
      # Time between endoscope images in ms
      time_diff = (new_measurement_time - self.endo_cam_psm1_last_measurement) * 1000
      print(f"Time between PSM1 endoscope images: {time_diff:.2f} ms")
      self.endo_cam_psm1_last_measurement = new_measurement_time
    else:
      self.endo_cam_psm1_last_measurement = time.time()
  ...
  def get_endo_cam_psm2(self,data):
    # TODO: Check if the image won't be received frequent enough
    if self.endo_cam_psm2_last_measurement is not None:
      new_measurement_time = time.time()
      # Time between endoscope images in ms
      time_diff = (new_measurement_time - self.endo_cam_psm2_last_measurement) * 1000
      print(f"Time between PSM2 endoscope images: {time_diff:.2f} ms")
      self.endo_cam_psm2_last_measurement = new_measurement_time
    else:
      self.endo_cam_psm2_last_measurement = time.time()
  ...
  def get_ecm_pose(self, data):
    # TODO: Check if the image won't be received frequent enough
    if self.ecm_last_measurement is not None:
      new_measurement_time = time.time()
      # Time between ECM images in ms
      time_diff = (new_measurement_time - self.ecm_last_measurement) * 1000
      print(f"Time between ECM kinematics: {time_diff:.2f} ms")
      self.ecm_last_measurement = new_measurement_time
    else:
      self.ecm_last_measurement = time.time()
